<template>
  ...
  <div>
    <div class="grid grid-cols-2 px-32 gap-32">
      <div class="" v-for="obj in mountain.results" :key="obj.id">
        <div
          class="max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700 "
        >
          <a href="#">
            <img class="rounded-t-lg w-full max-h-52" :src="obj.header_image.list" alt="" />
          </a>
    ...
    <div class="text-center mt-16">
      <nav aria-label="Page navigation example">
        <ul class="inline-flex -space-x-px">
          <li>
            <a
              :href="`/news/${mountain.previous.slice(33)}`"
          ...
          <li>
            <a
              :href="`/news/${mountain.next.slice(33)}`"
              @click="page"
...
{{mountain.previous.slice(33)}}
  </div>
</template>
<script setup>
const route = useRoute()
const page = route.params.number

const { data: mountain } = useFetch('http://127.0.0.1:8000/news/?page=' + page)
</script>
